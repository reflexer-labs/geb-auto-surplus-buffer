...
import "../MultiAutoSurplusBufferSetter.sol";
...
contract AccountingEngine {
    ...
    function surplusBuffer(bytes32) public view returns (uint256) {
        return buffer;
    }

    function modifyParameters(bytes32, bytes32 parameter, uint data) external {
        if (parameter == "surplusBuffer") buffer = data;
    }
}
contract SAFEEngine {
    uint256 debt;

    function globalDebt(bytes32) public view returns (uint256) {
        return debt;
    }
...
contract MultiAutoSurplusBufferSetterTest is DSTest {
    Hevm hevm;
    ...
    MultiAutoSurplusBufferSetter setter;
    ...
    function setUp() public {
        ...
        setter = new MultiAutoSurplusBufferSetter(
            coinName,
            address(treasury),
            address(safeEngine),
    ...
    function test_adjustSurplusBuffer() public {
        safeEngine.modifyParameters(coinName, "globalDebt", 500000000E45);
        setter.adjustSurplusBuffer(address(0));
        assertEq(accountingEngine.surplusBuffer(coinName), 25000000E45);
    }
    ...
    function test_adjustSurplusBuffer_result_above_max_limit() public {
        ...
        setter.adjustSurplusBuffer(address(0));
        assertEq(accountingEngine.surplusBuffer(coinName), 250000E45);
    }
    function testFail_adjustSurplusBuffer_positive_debt_change_under_threshold() public {
        safeEngine.modifyParameters(coinName, "globalDebt", 500000000E45);
        setter.adjustSurplusBuffer(address(0));
        hevm.warp(now + periodSize);
        safeEngine.modifyParameters(coinName, "globalDebt", 501000000E45);
        setter.adjustSurplusBuffer(address(0));
    }
    function testFail_adjustSurplusBuffer_negative_debt_change_under_threshold() public {
        ...
        setter.adjustSurplusBuffer(address(0));
        hevm.warp(now + periodSize);
        safeEngine.modifyParameters(coinName, "globalDebt", 499000000E45);
        setter.adjustSurplusBuffer(address(0));
    }
